...
2. **The container is negotiated per browser** (Opus/WebM, then MP4/AAC, then whatever
   the browser picks), and normalisation stays at the service boundary, not in the page.
...
6. **Time-to-first-audio in the browser is the whole turn.** Streaming the reply into the
   voice shortens the turn, not the wait for the first sound (§3).
...
Being liberal here is free because audio is normalised to 16 kHz mono PCM **at the service
boundary**: `services/audio.py` does it, through PyAV. The browser's job is to produce
something ffmpeg reads. Transcoding in the page would move work onto the weakest machine
in the system to save a step on the strongest.
...
Streaming the reply into the voice sentence by sentence — PRD §9.1's first fallback —
buys about 140 ms at the median, roughly a twentieth of a turn
([0003 §3](0003-conversation-context-strategy.md)).

**It does not bring the first sound forward in the browser.** `POST /sessions/{id}/turns`
concatenates the synthesised sentences into one WAV (`services/wav.py`) and stores it as a
single content-addressed asset. The browser receives one `audio_url` after the whole turn
has completed. So the first sound a user hears arrives at **turn latency**, not at
first-sentence latency, and the 78 ms figure of [0002](0002-tts-model-choice.md) describes a
boundary inside the API that nothing downstream can observe.

The overlap is still worth having — it shortens the turn, which is what the p95 budget is
measured against — but its benefit is entirely server-side.

**What bringing the first sound forward would take**, so this is a decision and not a
complaint:
...
That is a large change to recover a fraction of ~2.4 s, on a screen where the person has
just finished speaking and is not yet listening. It is **not** built. The honest
measurement to make first is whether a user notices the difference at all, which needs
users, not another endpoint.

**What the page does instead** is make the wait legible rather than shorter. The stage
...
A person held the button for a seven-turn conversation with the apartment-viewing persona,
on speech that was not LibriSpeech. It works: three user turns transcribed at confidence
0.90, 0.90 and 0.93, the persona in character throughout, every reply ending on a complete
sentence, all seven audio assets on disk.
...
expensive to transcribe, and a read-aloud passage, longer than a conversational turn, does
...
| | 0003, load 1.74 | this run, load 3.1 / 11.4 / 13.0 |
|---|---|---|
...
records, and its 16–30 s turns sit where that curve predicts. **No part of it is a defect in
the recorder and no part of it is the recordings being long.**

### What this actually means

**The 3000 ms budget is a quiet-machine number.** The turn meets it at load 1.74; a laptop
that is also running a Docker build and two other stacks does not. 0003 §3 records 7283 ms
at load 10–16, and a person using the product sees it too — the experience of a 25-second
turn is qualitatively different from a table row saying 7283.

Nothing is changed in response, for the reason PRD §9.1's own fallback list implies: every
remaining lever (a smaller recogniser, a shorter reply cap, a smaller model) trades quality
for a latency problem that the target machine does not have when it is doing one thing at a
time. The statement instead, here and in the README: **this product's latency is dominated
by what else is running on the machine**, and a number quoted without its load average is
not a measurement.
...
actually missing. That branch is confirmed in a real browser: a browser that reaches the
app on `host.docker.internal:3003`, which is not a secure origin, refuses to record and
says why.
...
There is no shared cookie jar between the browser and the Next.js server — `lib/auth.ts`
says so — so a server component fetching `/sessions/12` gets a 401 unless the header is
forwarded by hand.
...
- **A 401 and a 404 render the same thing.** The API answers 404 for somebody else's
  session so that guessing an id tells you nothing; a page that said "you are not allowed
  to see session 41" would hand back exactly the fact the server withheld.
...
recogniser then transcribes SpeakLab's own voice as though the learner had said it — the
wrong audio arriving through the room instead of through a codec, and invisible in the
data: a perfectly confident transcript of a sentence nobody spoke.
...
stopping playback automatically when recording starts — is rejected because it needs the
...
- **The voice changes.** Nothing here depends on which voice speaks, but the auto-play
  decision assumes the reply is worth listening to.
