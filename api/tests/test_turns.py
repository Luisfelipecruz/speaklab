...
async def test_a_turn_is_atomic_when_generation_fails(
    seeded, client, account, provider, voice, recogniser, audio_root, db_session
):
    ...
    **`provider` is in the fixture list and must stay there.** Without it the opening
    turn is generated by the *real* Ollama client, and this test then passes or fails
    depending on whether a model happens to be running on the machine — which is how it
    passed locally and failed on CI. The session has to start against a working stub so
    that the only thing broken is the thing under test.
    """
    from main import app
