...
def test_the_header_names_the_build_the_figures_describe():
    """A tag is a pointer the library can move; the digest is the build measured."""
    verdicts = report.adjudicate({}, skips={}, readme="")
    named = report.render(
        {},
        verdicts,
        revision="abc1234",
        model="`gemma4:e4b` build `c6eb396dbd59` (8.0B, Q4_K_M, 9.6 GB)",
    )
    unnamed = report.render({}, verdicts, revision="abc1234")

    assert (
        "Generated by `make eval` at revision `abc1234`, with `gemma4:e4b` build "
        "`c6eb396dbd59` (8.0B, Q4_K_M, 9.6 GB) as the language model." in named
    )
    assert "Generated by `make eval` at revision `abc1234`." in unnamed
