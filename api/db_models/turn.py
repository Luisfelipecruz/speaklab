...
`words` is where the fluency numbers come from: `[{w, start_ms, end_ms, logprob}]`
straight off the ASR. It is JSONB rather than a `words` table because the array is read whole,
...
`asr_confidence` is the gate: a turn the recogniser was unsure of must not contribute
to an accuracy trend, because an ASR error scored as a grammar error is a correction the
user cannot act on and a trend line that moves for the wrong reason.
"""
...
class Turn(Base):
    ...
    # Null for an assistant turn until its reply has been synthesised, and null for a
    # user turn whose owner set retain_audio = false and whose waveform has been deleted.
    ...
    # ── What produced an assistant turn ─────────────────────────────────────
    #
    ...
    # And which voice spoke it. Synthesis is non-deterministic, so the audio cannot be
    # re-derived from the text to find out after the fact — if this is not recorded at
    # the moment of synthesis it is not recoverable at all.
    tts_voice: Mapped[str | None] = mapped_column(Text)

    # Ollama's own count of what it read and what it wrote, taken from the response
    # rather than estimated. The history is bounded in tokens, and a bound nothing ever
    # measures is a bound nobody can show was respected: `services/conversation.py`
