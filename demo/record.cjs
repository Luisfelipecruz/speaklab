/**
 * The SpeakLab walkthrough — one capture, driven against the running stack.
 *
 ...
 *
 * It writes three things into out/:
 *
 *   speaklab-<format>.webm          the capture
 *   speaklab-<format>-poster.png    the thumbnail — a native render, not a video frame
 *   speaklab-<format>.meta.json     the marks: where the dead opening ends, and every
 *                                   measured latency in the take
 *
 * ── The microphone ────────────────────────────────────────────────────────
 *
 ...
 * files are lines written for the take and spoken by the project's own `tts` voice
 * (`voices.sh`), with the mistakes and hesitations the take is meant to show written into
 * them. Nothing downstream of the stream is touched: the clip goes through MediaRecorder,
 * the upload, Whisper, the persona model and Piper exactly as a live turn does, and the
 * stopwatch measures exactly that path.
 *
 ...
 * from release to that moment — the figure on the stopwatch.
 */
async function speak(page, clip, { during, after, until, mark }) {
  ...
  if (during) {
    await wait(page, Math.min(3000, clipMs / 3));
    await lab(page, during);
  }
  await wait(page, Math.max(0, clipMs + 350 - (during ? Math.min(3000, clipMs / 3) : 0)));
  await page.mouse.up();
...
async function pauseReply(page) {
  ...
  if (await pause.count()) await tap(page, pause, 250);
}
...
(async () => {
  ...
  if (nMarked > 0) {
    ...
    await poster(page, path.join(OUT, `speaklab-${FORMAT}-poster.png`));
    await wait(page, 1200);
    if (await page.getByText('may be a mishearing').count()) {
  ...
  await lab(page, 'The same synthetic voice, with a filler and a word said twice written into the answer');
  await wait(page, answerMs + 350);
  await page.evaluate(() => { window.__clip = null; });
  await lab(page, '');
  ...
  const final = path.join(OUT, `speaklab-${FORMAT}.webm`);
  ...
  fs.writeFileSync(path.join(OUT, `speaklab-${FORMAT}.meta.json`), JSON.stringify(MARKS, null, 2));

  console.log('VIDEO:', final, (fs.statSync(final).size / 1e6).toFixed(1) + ' MB', 'wallclock', at() + 's');
  console.log('MARKS:', JSON.stringify(MARKS));
  console.log(
    `NEXT:  SS=${MARKS.head} SCALE=${OUT_W}:${OUT_H} ` +
    `POSTER=speaklab-${FORMAT}-poster.png ./to-mp4.sh out/${path.basename(final)}`);
