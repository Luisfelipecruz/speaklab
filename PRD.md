...
| LLM reply (`gemma4:e4b`, ~80 tokens, host Ollama on Metal) | ≤ 1500 ms |
| TTS (Piper) | ≤ 400 ms |
...
graph TB
    ...
    OLL["Ollama — gemma4:e4b<br/>host :11434, Metal"]
...
| Conversation | `gemma4:e4b` (8B) via host Ollama, thinking off | With thinking off its reply is as fast as a 4B model's on Metal while holding a persona; with it on, seconds go on a thought nobody hears | `gemma3:4b` for a 3.3 GB download on a small disk |
...
| Error labelling | `gemma4:e4b`, constrained JSON | Proposes labels only from the closed taxonomy; a rule layer validates spans and drops anything unmatched (FR-19) |
...
| **Rehearse a presentation.** The learner pastes the script of a talk; it is split into sections of about a paragraph; each section is rehearsed as many times as they like, and every take is compared with the script word by word, timed and counted like a spoken answer, and scored sound by sound like a reading. Takes of the same section are shown side by side. Later, a listener persona built from the script asks the questions an audience would | Delivered as *Rehearse*, without the listener: the split is arithmetic and shown before it is saved, the words no converter can turn into phones are named to the writer rather than skipped, and nothing a take produces reaches the progress snapshots — `docs/decisions/0028`. A pasted script is user-authored content but not a scenario, which §15 keeps out of scope: no persona, no goal, no grammar targets. The listener is not built |
