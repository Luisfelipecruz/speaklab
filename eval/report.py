...
def render(
    ...
    `model` is the conversation model as the running API reported it — name, build
    digest, size — so a reader can tell which build the figures describe; a tag alone
    is a pointer that the library can move.
    """
    skips = skips or {}
    ...
    lines = [
        ...
        + (f", with {model} as the language model." if model else "."),
        "",
